								 ...
								 double realizedPNL, const CString &accountName) 
{
    if ( m_portfolio.m_hWnd != NULL ) {
	    ...
	    str.Format( "conId=%i symbol=%s  secType=%s  expiry=%s  strike=%f right=%s multiplier=%s primaryExchange=%s"
			" currency=%s localSymbol=%s position=%i mktPrice=%f mktValue=%f avgCost=%f unrealizedPNL=%f realizedPNL=%f account=%s",
			...
			(const char *)contract.currency, (const char *)contract.localSymbol,
